class TorreBianco extends PezzoBianco {
    ...
    calcolaMossePossibili(Scacchiera) {
        ...
        for (let i = this.y-1; i>=0; i--) {     //controlla tutte le celle con la stessa x sopra alla torre
            ...
            else if ($("td:eq(" + (this.x + 8 * i ) + ")").html().search("black") !== -1) {     //se nella cella è presente un pezzo nero aggiunge la mossa e termina il ciclo
                mossePossibili.push([this.x, i]);
                break;
            }
            else break; //se nella cella è presente un pezzo bianco termina il ciclo
        }
        ...
        for (let i = this.x+1; i<=7; i++) {     //controlla tutte le celle con la stessa y a destra alla torre
            ...
            else if ($("td:eq(" + (i + 8 * this.y ) + ")").html().search("black") !== -1) {  //se nella cella è presente un pezzo nero aggiunge la mossa e termina il ciclo
